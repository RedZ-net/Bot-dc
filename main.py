...
async def forward(ctx, *, args=None):
    ...
    try:
        ...
        if original_message.author.id != bot.user.id:
            await ctx.send("Can only forward messages sent by yourself.")
            return
    ...
    async def forward_loop():
        await dest_channel.send(original_message.content)
        while True:
            await asyncio.sleep(interval)
            await dest_channel.send(original_message.content)
    ...
    await ctx.send(f"Started forwarding to <#{channel_id}> every {interval} seconds.")
