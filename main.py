...
async def message(ctx, *, args=None):
    ...
    try:
        ...
        msg_text = msg_parts[0].strip()  # The message text
        channel_id = int(msg_parts[1].strip())  # The target channel ID
        interval = int(msg_parts[2].strip())  # The interval in seconds
    ...
    # Define the task to send the message and repeat it after the interval
    async def send_repeating():
        await channel.send(msg_text)  # Send the initial message immediately
        while True:
            await asyncio.sleep(interval)  # Wait for the interval time
            await channel.send(msg_text)  # Send the message again after the interval
...
async def stop(ctx, channel_id: int):
    ...
    if task:
        ...
        del tasks[channel_id]  # Remove it from the task dictionary
